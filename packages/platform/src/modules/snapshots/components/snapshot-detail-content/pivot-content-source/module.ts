...
@Module('SourceAnalyzeStatisticsModule')
export class SourceAnalyzeStatisticsModule extends EffectModule<State> {
  defaultState = {
    data: null,
    loading: false,
  }
  ...
  @Reducer()
  setData(state: Draft<State>, data: SourceAnalyzeStatistics): State {
    return {
      ...state,
      data: freeze(data),
    }
  ...
  fetchSourceCoverageResult(payload$: Observable<string>) {
    return payload$.pipe(
      switchMap((key) =>
        this.fetch.get<SourceAnalyzeStatistics>(key).pipe(
